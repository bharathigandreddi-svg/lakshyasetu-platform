...
(function(){
  ...
  function cleanupLegacySubjectActions(){
  ...
  function enhanceSubjectCatalog(){
    ...
    document.querySelectorAll('.subject-card .action-row').forEach(row=>{
      ...
      const viewTopic=document.createElement('a');viewTopic.className='buy-btn secondary';viewTopic.href=href;viewTopic.textContent='VIEW TOPIC →';viewTopic.setAttribute('data-topic-link','true');
      const buyButton=document.createElement('button');buyButton.type='button';buyButton.className='buy-btn subject-package-buy';buyButton.textContent='BUY ₹99 PACKAGE';buyButton.setAttribute('data-subject-buy','true');buyButton.addEventListener('click',()=>buy({product_type:'subject',course_subject_id:subjectId},'LakshyaSetu Subject Package · ₹99'));
      row.innerHTML='';row.append(buyButton,viewTopic);row.dataset.subjectEnhanced='1';
    });
    cleanupLegacySubjectActions();
  }
  function startCatalogEnhancer(){
    const run=()=>enhanceSubjectCatalog();
