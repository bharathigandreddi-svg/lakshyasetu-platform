...
(function(){
  ...
  async function invokePaymentFunction(client,functionName,body){
    ...
    async function call(s){
      return await client.functions.invoke(functionName,{body,headers:{Authorization:`Bearer ${s.access_token}`}});
    }
  ...
  async function buy(product,label,knownSession=null){
    ...
    const session=knownSession||await getAuthenticatedSession(client);
