...
(function(){
  ...
  async function invokePaymentFunction(client, functionName, body, session){
    return client.functions.invoke(functionName, {body,headers:{Authorization:`Bearer ${session.access_token}`}});
  }
    ...
    try{
      ...
      const rzp=new Razorpay({key:orderData.key_id,amount:orderData.order.amount,currency:orderData.order.currency||'INR',name:'LakshyaSetu',description:label||product.product_type,order_id:orderData.order.id,handler:async function(response){
        try{
          ...
          const {data,error}=await invokePaymentFunction(client,'verify-razorpay-payment',{...product,amount:orderData.amount,razorpay_order_id:response.razorpay_order_id,razorpay_payment_id:response.razorpay_payment_id,razorpay_signature:response.razorpay_signature},verifySession);
  ...
  function addButton(host,product,label,priceText){
    ...
    const b=document.createElement('button');b.className='action-btn ls-buy-btn';b.type='button';b.textContent=label+(priceText?' '+priceText:'');b.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();buy(product,label);};host.appendChild(b);
  }

  function enhanceCards(){
    document.querySelectorAll('.card').forEach(card=>{
      if(card.querySelector('.ls-buy-btn')) return;
      const onclick=card.getAttribute('onclick')||'';let product=null,label='Buy',fee='';
    ...
    document.querySelectorAll('.test-item').forEach(item=>{
      if(item.querySelector('.ls-buy-btn')) return;const open=item.querySelector('[onclick*="openTest("]');if(!open)return;
      const m=(open.getAttribute('onclick')||'').match(/openTest\((\d+)\)/);if(m){const price=item.querySelector('.fee')?.textContent?.trim()||'';addButton(item,{product_type:'test',test_id:Number(m[1])},'Buy Test',price&&price!=='Free'?price:'');}
    });
  }

  async function hasLegacyAccess(client,uid,topic){
    const {data:p1}=await client.from('purchases').select('id,expires_at').eq('user_id',uid).eq('payment_status','paid').eq('purchase_type','topic').eq('topic_id',topic.id).limit(1);
    const {data:p2}=await client.from('purchases').select('id,expires_at').eq('user_id',uid).eq('payment_status','paid').eq('purchase_type','subject').eq('subject_id',topic.course_subject_id).limit(1);
    const valid=(rows)=>Array.isArray(rows)&&rows.some(row=>!row.expires_at||new Date(row.expires_at)>new Date());return valid(p1)||valid(p2);
  }

  async function hasModernAccess(client,uid,test){
    const checks=[['test','test_id',test.id],['test_series','test_series_id',test.test_series_id],['test_package','test_package_id',test.package_id]];
    for(const [type,column,id] of checks){if(!id)continue;const {data}=await client.from('ls_purchases').select('id,access_expiry_at').eq('user_id',uid).eq('status','ACTIVE').eq('product_type',type).eq(column,id).limit(1);if(Array.isArray(data)&&data.some(row=>!row.access_expiry_at||new Date(row.access_expiry_at)>new Date()))return true;}return false;
  }
