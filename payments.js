/* LakshyaSetu Razorpay payment client */
(function(){
  ...
  async function session(){
    ...
    return rr.data?.session||null;
  }
  async function loadRazorpay(){
    if(window.Razorpay)return;
    await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://checkout.razorpay.com/v1/checkout.js';s.onload=resolve;s.onerror=()=>reject(new Error('Payment gateway failed to load.'));document.head.appendChild(s);});
  }
  async function invoke(name,body){
    const c=db();
    let s=await session();
    if(!s)throw new Error('Authentication failed. Please login again.');
    const call=()=>c.functions.invoke(name,{body});
    let r=await call();
    ...
    if(r.error)throw new Error(r.data?.error||r.error.message||'Payment service failed.');
    if(!r.data?.success)throw new Error(r.data?.error||'Payment service failed.');
    return r.data;
  }
  function loginModal(product,label){
    const email=prompt('Student email');
    if(!email)return;
    const password=prompt('Student password');
    if(!password)return;
    db().auth.signInWithPassword({email:email.trim(),password}).then(r=>{if(r.error)throw r.error;return buy(product,label);}).catch(e=>alert(e.message||'Unable to login.'));
  }
  async function buy(product,label,knownSession){
    try{
      const s=knownSession||await session();
      if(!s){loginModal(product,label);return;}
      await loadRazorpay();
      ...
      rzp.on('payment.failed',response=>{const e=response?.error||{};alert('Payment failed: '+(e.description||'Please retry.'));});
      rzp.open();
