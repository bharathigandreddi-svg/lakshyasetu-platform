...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  try{isGithubPages=origin?new URL(origin).hostname.toLowerCase().endsWith(".github.io"):false;}catch(_e){}
  const razorpayMode=keyId.startsWith("rzp_test_")?"test":keyId.startsWith("rzp_live_")?"live":"unknown";
  if(isGithubPages&&razorpayMode!=="test"){
   console.error("Razorpay mode mismatch",{origin,razorpayMode});
  ...
  const razor=btoa(`${keyId}:${keySecret}`);const r=await fetch("https://api.razorpay.com/v1/orders",{method:"POST",headers:{Authorization:`Basic ${razor}`,"Content-Type":"application/json"},body:JSON.stringify({amount,currency:"INR",receipt:`ls_${type}_${id}_${Date.now()}`,notes:{user_id:userData.user.id,product_type:type,product_id:String(id)},payment_capture:1})});const order=await r.json();
