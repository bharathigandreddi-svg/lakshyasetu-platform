...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const amountRupees=pickAmount(row,type==="test"?5:type==="course"?courseSeriesFallback(row):0);if(!Number.isFinite(amountRupees)||amountRupees<=0)return json({success:false,error:"This product has no active price."},400);const amount=Math.round(amountRupees*100);if(amount<100)return json({success:false,error:"Minimum payment amount is ₹1."},400);
