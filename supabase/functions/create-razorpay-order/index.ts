...
async function authenticate(req:Request,supabaseUrl:string){
  ...
  const publishableKey=envKey("SUPABASE_PUBLISHABLE_KEYS",["SUPABASE_PUBLISHABLE_KEY","SUPABASE_ANON_KEY"]);if(!publishableKey)return {userId:null,error:"Supabase authentication configuration is incomplete."};
  ...
  const user=await r.json();if(!user?.id)return {userId:null,error:"Invalid login session. Please login again."};return {userId:String(user.id),error:null};
}
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const serviceKey=envKey("SUPABASE_SECRET_KEYS",["SUPABASE_SECRET_KEY","SUPABASE_SERVICE_ROLE_KEY"]);
    ...
    const origin=req.headers.get("origin")||"";let isGithubPages=false;try{isGithubPages=origin?new URL(origin).hostname.toLowerCase().endsWith(".github.io"):false;}catch(_e){}
    ...
    const {createClient}=await import("https://esm.sh/@supabase/supabase-js@2");const admin=createClient(supabaseUrl,serviceKey,{auth:{persistSession:false,autoRefreshToken:false}});
    ...
    const fallback=type==="test"?2:type==="subject"?99:type==="course"?courseSeriesFallback(row):0;
    const amountRupees=pickAmount(row,fallback);if(!Number.isFinite(amountRupees)||amountRupees<=0)return json({success:false,error:"This product has no active price."},400);const amount=Math.round(amountRupees*100);if(amount<100)return json({success:false,error:"Minimum payment amount is ₹1."},400);
    const razor=btoa(`${keyId}:${keySecret}`);const r=await fetch("https://api.razorpay.com/v1/orders",{method:"POST",headers:{Authorization:`Basic ${razor}`,"Content-Type":"application/json"},body:JSON.stringify({amount,currency:"INR",receipt:`ls_${type}_${id}_${Date.now()}`,notes:{user_id:userId,product_type:type,product_id:String(id)},payment_capture:1})});const order=await r.json();
    ...
    const purchase:Record<string,unknown>={user_id:userId,product_type:type,amount:amountRupees,status:"PENDING",payment_status:"pending",razorpay_order_id:order.id};purchase[mapping[1]]=id;const {error:purchaseError}=await admin.from("ls_purchases").insert(purchase);if(purchaseError){console.error("Pending purchase insert failed",purchaseError);return json({success:false,error:"Unable to initialise the purchase. Please try again."},500);}
