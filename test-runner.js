(function(){
  'use strict';
  /* student-v2.html has the canonical embedded runner. Do not wrap its openTest(). */
  if(document.getElementById('runnerSection')) return;
  ...
  async function start(testId,fallbackOpen){
    ...
    if(!access){
      ...
      for(const [type,key,val] of checks){const q=await client.from('ls_purchases').select('id').eq('user_id',uid).eq('status','ACTIVE').eq(key,val).or('access_expiry_at.is.null,access_expiry_at.gte.'+new Date().toISOString()).limit(1);if(q.data?.length){access=true;break;}}
    }
    ...
    if(qe)throw qe;if(!questions?.length)throw new Error('Questions are not available yet.');
    const totalMarks=questions.reduce((s,q)=>s+Number(q.marks||1),0);
    const {data:attempt,error:ae}=await client.from('ls_test_attempts').insert({test_id:Number(test.id),user_id:uid,total_marks:totalMarks}).select().single();if(ae)throw ae;
    const duration=Number(test.duration_minutes||0),started=Date.now(),host=area();
    ...
    async function submit(){if(form.dataset.submitted==='1')return;form.dataset.submitted='1';if(timer)clearInterval(timer);const answers=[];let score=0,correct=0,wrong=0,unanswered=0;for(const q of questions){const selected=form.querySelector('input[name="q'+q.id+'"]:checked')?.value||null;const ok=selected&&String(selected).toUpperCase()===String(q.correct_option||'').toUpperCase();const marks=Number(q.marks||1),neg=Number(q.negative_marks||0);let awarded=0;if(!selected)unanswered++;else if(ok){correct++;awarded=marks}else{wrong++;awarded=-neg}score+=awarded;answers.push({attempt_id:Number(attempt.id),test_question_id:Number(q.id),selected_option:selected,is_correct:selected?!!ok:null,marks_awarded:awarded})}const pct=totalMarks?Math.max(0,score/totalMarks*100):0;const ar=await client.from('ls_test_answers').insert(answers);if(ar.error)throw ar.error;const ur=await client.from('ls_test_attempts').update({submitted_at:new Date().toISOString(),score,total_marks:totalMarks,correct_count:correct,wrong_count:wrong,unanswered_count:unanswered,percentage:Number(pct.toFixed(2))}).eq('id',attempt.id).eq('user_id',uid);if(ur.error)throw ur.error;host.innerHTML='<div class="card"><h3>'+esc(test.title)+'</h3><div class="learning-box"><strong>Test Submitted Successfully</strong><div class="test-meta"><span class="meta-label">Score: '+score+' / '+totalMarks+'</span><span class="meta-label">Percentage: '+Number(pct.toFixed(2))+'%</span><span class="meta-label">Correct: '+correct+'</span><span class="meta-label">Wrong: '+wrong+'</span><span class="meta-label">Unanswered: '+unanswered+'</span></div></div></div>}
  ...
  (async()=>{for(let i=0;i<60;i++){if(window.getLakshyaSetuDb&&typeof window.openTest==='function'){const original=window.openTest;window.openTest=function(id){start(Number(id),original).catch(e=>{console.error(e);const h=area();if(h)h.innerHTML='<div class="error">Unable to start test: '+esc(e.message||e)+'</div>'})};return}await sleep(250)}})();
})();
